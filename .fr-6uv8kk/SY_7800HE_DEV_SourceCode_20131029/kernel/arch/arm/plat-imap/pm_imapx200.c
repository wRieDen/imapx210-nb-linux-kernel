...
void imapx200_reset_peripheral(void)
{
	__raw_writel(0xffffffff, rAHBP_RST);
	__raw_writel(0xffffffff, rAPBP_RST);
	mdelay(100);
	__raw_writel(0, rAHBP_RST);
	__raw_writel(0, rAPBP_RST);
	mdelay(100);
}

void imapx200_pm_restore_core(void)
{
	imapx200_reset_peripheral();
...
#ifndef CONFIG_PM_SHUT_DOWN
	__raw_writel(0xff, rWP_MASK);
#else
	__raw_writel(0xfe, rWP_MASK);
#endif
