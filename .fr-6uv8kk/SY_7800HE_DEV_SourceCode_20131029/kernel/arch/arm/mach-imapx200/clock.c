...
/*
static struct clk clk_ext_camif = {
	.name 		= "camif-ext",
	.id   		= -1,
	.parent 	= &clk_epll,
	.enable		= imap_clkcon_enable,
	.ctrlbit       	= IMAP_CLKCON_SCLK_CAM,
};
*/
...
#else
	unsigned long clkdiv0, clkdiv2;
	/* set epll clock to 432MHz */
	__raw_writel(__raw_readl(rPLL_OCLKSEL)&(~(0x1<<2)), rPLL_OCLKSEL);
	__raw_writel(__raw_readl(rEPLL_CFG)&(~(0x1<<31)), rEPLL_CFG);
	__raw_writel(0x80001023, rEPLL_CFG);
	while(!(__raw_readl(rPLL_LOCKED)>>2));
	__raw_writel(__raw_readl(rPLL_OCLKSEL)|(0x1<<2), rPLL_OCLKSEL);
