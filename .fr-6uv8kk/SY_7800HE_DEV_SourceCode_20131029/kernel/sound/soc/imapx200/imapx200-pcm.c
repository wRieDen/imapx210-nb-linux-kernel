...
//extern imapx200_dma_ch imapx200_pch[2];
int sound_dma_irq = 0;
...
extern struct imapx200_dma_ch imapx200_pch[2];
extern struct imapx200_dma_ch imapx200_pch_in[2];
	...
	struct snd_pcm_hw_params *params)
{
	...
	struct imapx200_pcm_dma_params *dma; 
	unsigned long totbytes = params_buffer_bytes(params);
	int ret = 0;
	/*
	struct imapx200_dma_ch pch_new [] = {
		{4096,0,32,32,1,1,0,1,0,0,0,0,0,0},
		{4096,0,32,32,1,1,0,1,0,0,0,0,0,0},
	};
	*/
	pr_debug("Entered %s\n", __func__);
	...
	if (prtd->params == NULL) {
		/* prepare DMA */
		prtd->params = dma;
	//	prtd->params->channel->pch = &pch_new;
...
//	imapx200_dma_ctrl(prtd->params->channel, IMAPX200_DMAOP_STOP);
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
	imapx200_pch[0].src_addr = prtd->dma_start;
	imapx200_pch[1].src_addr = prtd->dma_start + 0x1000;
	printk(KERN_DEBUG "----src_addr0 is %x, src_addr1 is %x\n", imapx200_pch[0].src_addr, imapx200_pch[1].src_addr);
	}else {
	imapx200_pch_in[0].dst_addr = prtd->dma_start;
	imapx200_pch_in[1].dst_addr = prtd->dma_start + 0x1000;
	printk(KERN_DEBUG "----dst_addr0 is %x, dst_addr1 is %x\n", imapx200_pch_in[0].dst_addr, imapx200_pch_in[1].dst_addr);
...
static int imapx200_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		imapx200_pcm_prepare(substream);
...
imapx200_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		if (sound_dma_irq == 1)
		{
			...
			if (res >= 0x1000){
				//printk("*******pointer is 2\n");
				sound_dma_irq_index = 2;
			res = 0x1000;	
			}
			else{
				//printk("*******pointer is 1\n");
				sound_dma_irq_index = 1;
			res = 0x00;	
			}
		}else {
			
			if (sound_dma_irq_index ==2)
				res = 0x1000;
			else if (sound_dma_irq_index ==1)
				res = 0x0;
			else 
				res = 0;
