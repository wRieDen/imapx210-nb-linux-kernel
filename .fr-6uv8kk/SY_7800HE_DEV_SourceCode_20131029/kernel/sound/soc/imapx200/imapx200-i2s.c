				...
				struct snd_soc_dai *socdai)
{
	...
	struct imapx200_pcm_dma_params *dma_data;
	imapx200("Entered %s\n", __FUNCTION__);
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		dma_data = &imapx200_i2s_pcm_stereo_out;
	} else {
		dma_data = &imapx200_i2s_pcm_stereo_in;
	}
...
static int imapx200_i2s_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		printk(KERN_DEBUG "trigger command is %d\n", cmd);
	...
	int clk_id, unsigned int freq, int format)
{
	...
	printk(KERN_DEBUG"i2s clkdiv = %d, cdclkdiv = %d\n", iisclkdiv, cdclkdiv);
#endif
	/***********************************************************/
	rI2S_PRESCALER_value = readl(imapx200_i2s.regs + rI2S_PRESCALER);
	rI2S_PRESCALER_value = ((cdclkdiv<<16) | iisclkdiv);
//	writel(rI2S_PRESCALER_value, imapx200_i2s.regs + rI2S_PRESCALER);
	...
	if (freq == 44100){
		/* mclk=11.8M, bclk=2.8M, lrc=44.4K */
		clkdiv2 = __raw_readl(rDIV_CFG2);
		clkdiv2 &= ~((0x3<<24)| (0x7<<26));
		clkdiv2 |= (0x2<<24)| (0x1<<26);
		__raw_writel(clkdiv2, rDIV_CFG2);
		writel(((0x1<<26)| (0x1<<24)|(18<<16)|75), imapx200_i2s.regs + rI2S_PRESCALER);
	}
...
static int imapx200_i2s_probe(struct platform_device *pdev)
{
	...
	imapx200_i2s_set_sysclk(cpu_dai,22579200,44100,13);
	//printk(KERN_DEBUG"**********   iis_clock_rate is %x\n", imapx200_i2s.iis_clock_rate);
...
int imapx200_i2s_resume(struct platform_device *pdev,
	struct snd_soc_cpu_dai *dai)
{
	imapx200("Entered %s\n", __FUNCTION__);
...
//	imap_iokey_spken(0);
	...
	writel(0x0, imapx200_i2s.regs + rIER);;
	writel(0x0, imapx200_i2s.regs + rIRER);
	writel(0x0, imapx200_i2s.regs + rITER);
	writel(0x0, imapx200_i2s.regs + rCER);
...
/*
	...
	clk_enable(imapx200_i2s.iis_clk);
	imapx200_i2s.iis_clock_rate = clk_get_rate(imapx200_i2s.iis_clk);
	imapx200_i2s_set_sysclk(dai,384,44100,2);
*/
...
EXPORT_SYMBOL(imapx200_i2s_resume);
