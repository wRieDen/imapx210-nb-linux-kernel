...
static struct rt5621_reg init_data[] = {
	...
	{RT5621_STEREO_DAC_VOL,         0x6808},//0x6606},    //default stereo DAC volume to 3db
	{RT5621_HP_OUT_VOL,		0x8080},//0x8282},
	{RT5621_OUTPUT_MIXER_CTRL,     0x700},// 0x0700},    //default output mixer control
	{RT5621_ADC_REC_GAIN,	       0xf78f},//	0xff9f},    //set record to 30db gain
//	{RT5621_ADC_REC_GAIN,	       0xf811},//	0xff9f},    //set record to 30db gain
	{RT5621_MIC_CTRL,              0x0800},//// m701sa-- 0x0800},// 0x0000},    //set Mic1 boost 0db
	{RT5621_MIC_ROUTING_CTRL,	0xf0f0},    //set Mic1, Mic2 differential input control
	{RT5621_ADD_CTRL_REG,           0x5f00},//0x5f00},    //Class AB/D speaker ratio is 1VDD
...
#ifdef CONFIG_CODEC_ALC5621_ENABLE_JACK_DETECT
static char jack_high_output[] = CONFIG_CODEC_ALC5621_JD_H_OUT;
static char jack_low_output[] = CONFIG_CODEC_ALC5621_JD_L_OUT;
#endif

#define RT5621_GPIO	0
#define RT5621_JD1	1
#define RT5621_JD2	2
...
static int rt5621_ChangeCodecPowerStatus(struct snd_soc_codec *codec,int power_state)
{
	...
	switch(power_state)
	{
		...
		case POWER_STATE_D1_PLAYBACK:	//Low on of Playback
			...
			rt5621_write_mask(codec,RT5621_PWR_MANAG_ADD2,PWR_VREF|PWR_DAC_REF_CIR|PWR_L_DAC_CLK|PWR_R_DAC_CLK|PWR_L_HP_MIXER|PWR_R_HP_MIXER|PWR_SPK_MIXER|PWR_CLASS_AB|PWR_CLASS_D
											  			 ,PWR_VREF|PWR_DAC_REF_CIR|PWR_L_DAC_CLK|PWR_R_DAC_CLK|PWR_L_HP_MIXER|PWR_R_HP_MIXER|PWR_SPK_MIXER|PWR_CLASS_AB|PWR_CLASS_D);
// shaft 0225 (S)  //roy 20110614 open spk mixer power
			rt5621_write_mask(codec,RT5621_PWR_MANAG_ADD3,PWR_MAIN_BIAS|PWR_HP_R_OUT_VOL|PWR_HP_L_OUT_VOL|PWR_SPK_OUT 
											   			 ,PWR_MAIN_BIAS|PWR_HP_R_OUT_VOL|PWR_HP_L_OUT_VOL|PWR_SPK_OUT);		
	//		rt5621_write_mask(codec,RT5621_PWR_MANAG_ADD3,PWR_MAIN_BIAS|PWR_HP_R_OUT_VOL|PWR_HP_L_OUT_VOL
...
void externel_rt5621_AudioOutEnable(int mute)
{
	struct snd_soc_device *socdev = rt5621_socdev;
	struct snd_soc_codec *codec = socdev->card->codec;
	printk(KERN_INFO "externel_rt5621_AudioOutEnable(%d)++ \n",mute);
	...
	if (mute)
	{
		rt5621_AudioOutEnable(codec,RT_WAVOUT_SPK,1);	
		rt5621_AudioOutEnable(codec,RT_WAVOUT_HP,1);
		rt5621_write(codec,RT5621_STEREO_DAC_VOL,0xe808);
		rt5621_write(codec,RT5621_PWR_MANAG_ADD3,0);	
	}
	else
	{
		rt5621_AudioOutEnable(codec,RT_WAVOUT_SPK,0);	
		rt5621_AudioOutEnable(codec,RT_WAVOUT_HP,0);
		rt5621_write(codec,RT5621_STEREO_DAC_VOL,0x6808);	
	}

}
EXPORT_SYMBOL(externel_rt5621_AudioOutEnable);
...
//			printk("rt5621_prepare to STREAM_PLAYBACK\n");
...
//			printk("rt5621_prepare to STREAM_CAPTURE\n");
...
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct rt5621_priv *rt5621 = codec->drvdata;
//	printk("**************freq is %d\n\n\n\n\n\n",freq);
	switch (freq) {
	case 16934400:
#if 1
...
//	printk("rt5621_pcm_hw_params\n");
	if (coeff < 0)
		...
		coeff = get_coeff(22579200, params_rate(params));	  /*if not set sysclk, default to be 24.576MHz*/
	/* bit size */
//	printk("22222222222222222222222***********params_format(params) is %d \n\n\n\n\n",params_format(params));
	...
	case SNDRV_PCM_FORMAT_S20_3LE:
		//printk("SNDRV_PCM_FORMAT_S20_LE\n");	
		iface |= 0x0004;
		break;
	case SNDRV_PCM_FORMAT_S24_LE:
		//printk("SNDRV_PCM_FORMAT_S24_LE\n");	
		iface |= 0x0008;
		break;
	...
	//flove060811	return -EINVAL;
		
	}
	return 0;
}

static int rt5621_mute(struct snd_soc_dai *dai, int mute)
{
	u16 tmp;
	unsigned int reg;
	unsigned int index;
...
static void rt5621_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *codec_dai)
{
	...
	{
		case SNDRV_PCM_STREAM_PLAYBACK:
//			printk("rt5621_shutdown to STREAM_PLAYBACK\n");
			...
			break;
		case SNDRV_PCM_STREAM_CAPTURE:
//			printk("rt5621_shutdown to STREAM_CAPTURE\n");
...
#define RT5621_HIFI_RATES (SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_11025 |\
...
extern int imapx200_i2s_resume(struct platform_device *pdev,
...
//flove022211_S	
	...
	for (i = 0; i < ARRAY_SIZE(rt5621_reg); i++) {
		...
		printk("[%d]: codec reg[%02x] write 0x%02x%02x\n", i, data[0], data[1], data[2]);
...
#ifdef CONFIG_CODEC_ALC5621_ENABLE_JACK_DETECT
	if (CONFIG_CODEC_ALC5621_JD_SELECT == RT5621_GPIO)
	{
		rt5621_write(codec, RT5621_GPIO_PIN_SHARING, 0x0000);
		temp |= 0x1<<14;
	}
	if (CONFIG_CODEC_ALC5621_JD_SELECT == RT5621_JD1)
	{
		rt5621_write(codec, RT5621_GPIO_PIN_SHARING, 0x8000);
		temp |= 0x2<<14;
	}
	if (CONFIG_CODEC_ALC5621_JD_SELECT == RT5621_JD2)
	{
		rt5621_write(codec, RT5621_GPIO_PIN_SHARING, 0x4000);
		temp |= 0x3<<14;
	}

	if (jack_high_output[0] == '1')
		temp |= (0x1<<11);
	if (jack_high_output[1] == '1')
		temp |= (0x1<<10);
	if (jack_high_output[2] == '1')
		temp |= (0x1<<9);
	if (jack_high_output[3] == '1')
		temp |= (0x1<<8);

	if (jack_low_output[0] == '1')
		temp |= (0x1<<7);
	if (jack_low_output[1] == '1')
		temp |= (0x1<<6);
	if (jack_low_output[2] == '1')
		temp |= (0x1<<5);
	if (jack_low_output[3] == '1')
		temp |= (0x1<<4);

	rt5621_write(codec, RT5621_JACK_DET_CTRL, temp);
#endif
