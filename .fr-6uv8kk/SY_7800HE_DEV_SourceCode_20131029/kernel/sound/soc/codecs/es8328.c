	...
	unsigned int value)
{
	...
	printk(KERN_ALERT "es8328_write fail");
	return 1;
...
//	printk("*********now, init every register!\n");
	...
	//es8328_read(codec, ES8328_MASTERMODE);
	if(es8328_write(codec, ES8328_CHIPPOWER, 0xf3)) {
		printk(KERN_INFO "es8328_chippower 0xf3 fail try again");
		es8328_write(codec, ES8328_CHIPPOWER, 0xf3);
	}  //0x02
	//es8328_read(codec, ES8328_CONTROL1);
	es8328_write(codec,ES8328_CONTROL1 , 0x05);  //0x00
	//es8328_read(codec, ES8328_CHIPPOWER);

//es8328_write(codec, ES8328_CONTROL2, 0x040);	// 0x01
	
	es8328_write(codec, ES8328_CONTROL2, 0x070);	// 0x01, low power
	es8328_write(codec, ES8328_CHIPLOPOW1, 0x0fb);	// 0x05, low power
	es8328_write(codec, ES8328_CHIPLOPOW2, 0x0ff);	// 0x06 low power
	es8328_write(codec, ES8328_ANAVOLMANAG, 0x07a);	// 0x07 low power
	
	es8328_write(codec, ES8328_ADCPOWER, 0x0000);	// 0x03

	es8328_write(codec, ES8328_DACPOWER,  0x30);	// 0x04
	es8328_write(codec, ES8328_ADCCONTROL1,  0x88);	// 0x09
	...
	//es8328_write(codec, ES8328_ADCCONTROL4,  0x10);  //0x0c
	es8328_write(codec, ES8328_ADCCONTROL5,  0x02);//0x0d
	//es8328_write(codec, ES8328_ADCCONTROL5,  0x00);//0x0d
	es8328_write(codec, ES8328_ADCCONTROL7,  0xf0);
	es8328_write(codec, ES8328_ADCCONTROL8, 0x00);//0x10
	es8328_write(codec, ES8328_ADCCONTROL9, 0x00);//0x11

	es8328_write(codec, ES8328_ADCCONTROL10,  0xea);//0x12

	
	es8328_write(codec, ES8328_ADCCONTROL11, 0xc0); //0x13
	es8328_write(codec, ES8328_ADCCONTROL12, 0x12); //0x14
	es8328_write(codec, ES8328_ADCCONTROL13, 0x06); //0x15
//	es8328_write(codec, ES8328_ADCCONTROL14, 0xc1); //0x16
	es8328_write(codec, ES8328_ADCCONTROL14, 0xbb); //0x16
	es8328_write(codec, ES8328_DACCONTROL1, 0x18); //0x17
	//es8328_write(codec, ES8328_DACCONTROL1, 0x20); //0x17
	es8328_write(codec, ES8328_DACCONTROL2, 0x02); //0x18
	es8328_write(codec, ES8328_DACCONTROL4, 0x08); //0x1a
	es8328_write(codec, ES8328_DACCONTROL5, 0x08); //0x1b
	...
	es8328_write(codec, ES8328_DACCONTROL24, 0x1e); //0x2e
	es8328_write(codec, ES8328_DACCONTROL25, 0x1e); //0x2f
	if(es8328_write(codec, ES8328_CHIPPOWER, 0x00)){
		printk(KERN_ALERT "ES8328_CHIPPOWER fail,try once more.");
...
static int es8328_init(struct snd_soc_device *socdev)
{
	...
	ret = snd_soc_register_dai(&es8328_dai);
	if (ret != 0) {
