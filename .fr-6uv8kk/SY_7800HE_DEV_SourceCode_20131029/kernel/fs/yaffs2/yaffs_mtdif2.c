				      ...
				      const yaffs_ExtendedTags *tags)
{
	...
	int packed_tags_size = dev->param.noTagsECC ? sizeof(ptex.t) : sizeof(ptex);
	void * packed_tags_ptr = dev->param.noTagsECC ? (void *) &ptex.t : (void *)&ptex;
...
#if CONFIG_YAFFS_4BIT_TAGS_ECC
		yaffs_PackTags2(&ptex, tags, !dev->param.noTagsECC);
		ptexNand.t.sequenceNumber = ptex.t.sequenceNumber;
		ptexNand.t.objectId = ptex.t.objectId;
		ptexNand.t.chunkId = ptex.t.chunkId;
		ptexNand.t.byteCount = ptex.t.byteCount;		
...
#if CONFIG_YAFFS_4BIT_TAGS_ECC
	ops.ooblen = (dev->param.inbandTags) ? 0 : sizeof(ptexNand);
#else
...
#if CONFIG_YAFFS_4BIT_TAGS_ECC
	ops.oobbuf = (dev->param.inbandTags) ? NULL : (void *)&ptexNand;
#else
				       ...
				       __u8 *data, yaffs_ExtendedTags *tags)
{
	...
	if (dev->param.inbandTags || (data && !tags))
		...
		ops.len = data ? dev->nDataBytesPerChunk : sizeof(ptexNand);
#else
	...
	if (dev->param.inbandTags) {
		...
		if (tags) {
#if CONFIG_YAFFS_4BIT_TAGS_ECC
			memcpy(&ptexNand, yaffs_DeviceToLC(dev)->spareBuffer, sizeof(ptexNand));
			ptex.t.sequenceNumber = ptexNand.t.sequenceNumber;
	                ptex.t.objectId = ptexNand.t.objectId;
	                ptex.t.chunkId = ptexNand.t.chunkId;
	                ptex.t.byteCount = ptexNand.t.byteCount;
        	        memcpy(&(ptex.ecc), &(ptexNand.ecc), sizeof(yaffs_4bit_ecc));	

			yaffs_UnpackTags2(tags, &ptex, !dev->param.noTagsECC);
#else
