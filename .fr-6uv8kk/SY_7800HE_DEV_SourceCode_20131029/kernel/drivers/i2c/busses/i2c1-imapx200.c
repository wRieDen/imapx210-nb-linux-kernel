...
static void imapx200_i2c0_recover()
{
	...
	scl1 = imapx_gpio_getpin(IMAPX_GPC2, IG_NORMAL);
	sda1 = imapx_gpio_getpin(IMAPX_GPC3, IG_NORMAL);
	...
	for( i == 0; i < 10; i++ ) {
		...
		// change to scl0
		imapx_gpio_setcfg(IMAPX_GPC2, IG_OUTPUT, IG_NORMAL);
		imapx_gpio_setpin(IMAPX_GPC2, 0, IG_NORMAL);
		udelay(10);
		imapx_gpio_setpin(IMAPX_GPC2, 1, IG_NORMAL);
		udelay(10);
		imapx_gpio_setpin(IMAPX_GPC2, 0, IG_NORMAL);
		udelay(10);
		imapx_gpio_setpin(IMAPX_GPC2, 1, IG_NORMAL);
		...
		// change to scl0
		imapx_gpio_setcfg(IMAPX_GPC2, IG_CTRL0, IG_NORMAL);
		...
		struct i2c_msg *msgs, int num)
{
	...
	if (timeout == 0) {
		printk("i2c1 : timeout i2c->msg_num is %d addr 0x%x\n",i2c->msg_num, msgs->addr);
		imapx200_i2c0_recover();
		i2c->abort=0;
		i2c->dirty_bit = 1;
		if(i2c->msg)
...
//	printk("imapx200_i2c_irq i2c0 : irq = 0x%x,status = 0x%x mask =0x%x msg_num %d\r\n",irq,status,mask,i2c->msg_num);
...
static int imapx200_i2c_init(struct imapx200_i2c *i2c)
{
	...
	imapx_gpio_setcfg(IMAPX_GPC2, IG_CTRL0, IG_NORMAL);
	imapx_gpio_setcfg(IMAPX_GPC3, IG_CTRL0, IG_NORMAL);
	return 0;
}
