...
static char gt0308_write_cmos(uint8_t addr, uint8_t val)
{
	struct	gt0308_regval_list  reg;
	int ret;

	 reg.reg = addr;
	 reg.value = val;
	 ret = cam_gt0308_write( (unsigned char *)(&reg));
	 if(ret)
	 {
...
int gt0308_xx_start(void){
	...
	buf = 0;
	if(CMOSID == 0x99){
		for(i = 0; i < ((sizeof(gt0307_init_regs) / 2) -1); i++)
		{
			ret = cam_gt0308_write( (unsigned char *)(&gt0307_init_regs[i]));
			if(ret)
			{
				IC_ERR("Failed to transfer data to i2c\n");
				return -1;
...
int gt0308_xx_switch_high_xuga(void){ 
	...
	if(CMOSID == 0x99){
		for(i = 0; i < (sizeof(gt0307_xuga_high_regs) / 2); i++)
		{
			ret = cam_gt0308_write( (unsigned char *)(&gt0307_xuga_high_regs[i]));
			if(ret)
			{
				IC_ERR("Failed to transfer data to i2c\n");
				return -1;
		...
		for(i = 0; i < (sizeof(gt0308_xuga_high_regs) / 2); i++)
		{
...
int gt0308_xx_switch_mid_xuga(void){ 
	int i, ret;
	if(CMOSID == 0x99){
		for(i = 0; i < (sizeof(gt0307_xuga_mid_regs) / 2); i++)
		{
			ret = cam_gt0308_write( (unsigned char *)(&gt0307_xuga_mid_regs[i]));
			if(ret)
			{
				IC_ERR("Failed to transfer data to i2c\n");
				return -1;
...
int gt0308_xx_switch_low_xuga(void){ 
	int i, ret;
	if(CMOSID == 0x99){
		for(i = 0; i < (sizeof(gt0307_xuga_low_regs) / 2); i++)
		{
			ret = cam_gt0308_write( (unsigned char *)(&gt0307_xuga_low_regs[i]));
			if(ret)
			{
				IC_ERR("Failed to transfer data to i2c\n");
				return -1;
...
int gt0308_xx_to_preview_320_240(void){ 
	int i, ret;
	if(CMOSID == 0x99) {
		for(i = 0; i < (sizeof(gt0307_to_preview_320_240_regs) / 2); i++)
		{
			ret = cam_gt0308_write( (unsigned char *)(&gt0307_to_preview_320_240_regs[i]));
			if(ret)
			{
				IC_ERR("Failed to transfer data to i2c\n");
				return -1;
...
int gt0308_xx_to_preview_640_480(void){ 
	int i, ret;
	if(CMOSID == 0x99) {
		for(i = 0; i < (sizeof(gt0307_to_preview_640_480_regs) / 2); i++)
		{
			ret = cam_gt0308_write( (unsigned char *)(&gt0307_to_preview_640_480_regs[i]));
			if(ret)
			{
				IC_ERR("Failed to transfer data to i2c\n");
				return -1;
...
int gt0308_xx_sepia(void){ 
	...
	if(CMOSID == 0x99){
		uint8_t val;
		val = gt0308_read_cmos(0x41);
		val |= 0x39;
		gt0308_write_cmos(0x41,val);
		gt0308_write_cmos(0x40,0x7e);
		gt0308_write_cmos(0x42,0x10);
		gt0308_write_cmos(0x47,0x3c);
		gt0308_write_cmos(0x48,0xc3);
		gt0308_write_cmos(0x8a,0x60);
		gt0308_write_cmos(0x8b,0x60);
		gt0308_write_cmos(0x8c,0x07);
		gt0308_write_cmos(0x50,0x0c);
		gt0308_write_cmos(0x77,0x80);
		gt0308_write_cmos(0xa1,0x40);
		gt0308_write_cmos(0x7a,0x00);
		gt0308_write_cmos(0x78,0xc0);
		gt0308_write_cmos(0x79,0x20);
		gt0308_write_cmos(0x7b,0x40);
		gt0308_write_cmos(0x7c,0x00);
		gt0308_write_cmos(0xff,0xff);
...
int gt0308_xx_bluish(void){ 
	...
	if(CMOSID == 0x99){
		...
		gt0308_write_cmos(0xa1,0x40);
		gt0308_write_cmos(0x7a,0x00);
		gt0308_write_cmos(0x78,0x70);
		gt0308_write_cmos(0x79,0x00);
		gt0308_write_cmos(0x7b,0x3f);
		gt0308_write_cmos(0x7c,0xf5);
		gt0308_write_cmos(0xff,0xff);
	}else {
...
int gt0308_xx_reddish(void){
	...
	if(CMOSID == 0x99){
		...
		gt0308_write_cmos(0xa1,0x40);
		gt0308_write_cmos(0x7a,0x00);
		gt0308_write_cmos(0x78,0x80);
		gt0308_write_cmos(0x79,0x10);
		gt0308_write_cmos(0x7b,0x40);
		gt0308_write_cmos(0x7c,0x00);
		gt0308_write_cmos(0xff,0xff);
	} else {
...
int gt0308_xx_normal(void){ 
	...
	if(CMOSID == 0x99){
		...
		gt0308_write_cmos(0x47,0x2c);
		gt0308_write_cmos(0x48,0xc3);
		gt0308_write_cmos(0x8a,0x50);
		gt0308_write_cmos(0x8b,0x50);
		gt0308_write_cmos(0x8c,0x07);
		gt0308_write_cmos(0x50,0x0c);
		gt0308_write_cmos(0x77,0x80);
		gt0308_write_cmos(0xa1,0x40);
...
int gt0308_xx_auto(void){ 
	...
	if(CMOSID == 0x99){
		...
		val |= 0x04;
		gt0308_write_cmos(0x41,val);
		gt0308_write_cmos(0xc7, 0x4c);
		gt0308_write_cmos(0xc8, 0x40);       
		gt0308_write_cmos(0xc9, 0x4a);
		gt0308_write_cmos(0xff, 0xff);
	}else {
...
int gt0308_xx_sunny(void){ 
	...
	if(CMOSID == 0x99){
		...
		gt0308_write_cmos(0xc7, 0x50);
		gt0308_write_cmos(0xc8, 0x45);        
		gt0308_write_cmos(0xc9, 0x40);
		gt0308_write_cmos(0xff, 0xff);
	}else{
...
int gt0308_xx_cloudy(void){ 
	...
	if(CMOSID == 0x99){
		...
		val &= ~0x04;
		gt0308_write_cmos(0x41,val);
		gt0308_write_cmos(0xc7, 0x5a);
		gt0308_write_cmos(0xc8, 0x42);       
		gt0308_write_cmos(0xc9, 0x40);
		gt0308_write_cmos(0xff, 0xff);
	}else{
...
int gt0308_xx_office(void){ 
	...
	if(CMOSID == 0x99){
		...
		val &= ~0x04;
		gt0308_write_cmos(0x41,val);
		gt0308_write_cmos(0xc7, 0x40);
		gt0308_write_cmos(0xc8, 0x42);       
		gt0308_write_cmos(0xc9, 0x50);
		gt0308_write_cmos(0xff, 0xff);
	}else {
...
int gt0308_xx_home(void){ 
	...
	if(CMOSID == 0x99) {
		...
		val &= ~0x04;
		gt0308_write_cmos(0x41,val);
		gt0308_write_cmos(0xc7, 0x48);
		gt0308_write_cmos(0xc8, 0x50);        
		gt0308_write_cmos(0xc9, 0x5c);
		gt0308_write_cmos(0xff, 0xff);
	} else {
...
int gt0308_xx_set_effect(int value)
{
	...
	if(CMOSID == 0x99){
		uint8_t valu;
		uint8_t val_41,val_47;
		val_41 = gt0308_read_cmos(0x41);
		val_47 = gt0308_read_cmos(0x47);
		printk("**0x41 = %d,0x47 = %d\n",val_41,val_47);
		valu = gt0308_read_cmos(0x41);
		if (valu > 0x5f)
			gt0308_write_cmos(0x41,0x2c);
	}
