...
static void dpm_resume(pm_message_t state)
{
	...
	while (!list_empty(&dpm_list)) {
		...
		if (resume_time > max_time) {
			strncpy(name, dev_name(dev), strlen(dev_name(dev)));
			name[strlen(dev_name(dev))] = '\0';
			max_time = resume_time;
		}
#endif
	...
	mutex_unlock(&dpm_list_mtx);
#if defined(CONFIG_RESUME_BLOCK_DEBUG)
	printk(KERN_ALERT "\n[RESUME DEBUG] Longest resume device %s, cost %ds.\n",
			name, max_time);
#endif
