...
#ifdef	 CONFIG_SDI0_USE_EPLL_CLOCK
#define EPLL_CLK	1
#else 
#define EPLL_CLK        0
#endif
...
static void imapx_set_gpio(int channel,int width)
{
	switch(channel)
	{
		case 0:
			if(width == 4)
			{
				imapx_gpio_setcfg(IMAPX_GPF_RANGE(0, 5), IG_CTRL0, IG_NORMAL);
				imapx_gpio_pull(IMAPX_GPF_RANGE(0, 5), 0, IG_NORMAL);
			}
			else if(width == 0)
			{
...
/******************************************************************/
static struct mmc_host *sdhci_host0 = NULL;

void sdhci_mmc0_detect_change(void)
{
	if(sdhci_host0)
		mmc_detect_change(sdhci_host0,msecs_to_jiffies(1000));
}
EXPORT_SYMBOL(sdhci_mmc0_detect_change);
//===============================================================
...
static int __devinit sdhci_imap_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "[SDHCI0]:infoTM sdhci host driver init.\n");
	...
	host->quirks |= SDHCI_QUIRK_NO_BUSY_IRQ|SDHCI_QUIRK_FORCE_1_BIT_DATA;
...
static int __devexit sdhci_imap_remove(struct platform_device *pdev)
{
	...
	struct sdhci_imap *sc = sdhci_priv(host);
	int ptr;
