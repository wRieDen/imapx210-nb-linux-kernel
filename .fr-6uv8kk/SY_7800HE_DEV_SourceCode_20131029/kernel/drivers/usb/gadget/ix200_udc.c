...
static uint32_t g_rx_flag[IX_EP_COUNT];
...
static struct task_struct *gadget_task;
...
/* Wakes up otg thread*/
static DECLARE_WAIT_QUEUE_HEAD(otg_wait);
...
static uint32_t __b_connect(void)
{
	...
	if((readl(__reg + USB_BCSR) & USB_B_Valid))
	{
		...
		writel(0x00000080, __reg + USB_TBCR2);
		writel(0x00000080, __reg + USB_TBCR3);
...
int monitor_connect_thread(void *foo)
{
	...
	while(thread_state != THREAD_STOP)
	{
		...
		wait_event_freezable(otg_wait, (thread_wakeup == 1) || kthread_should_stop());
		thread_wakeup = 0;
//		sleep_thread();
	}
...
{
	gadget_task = kthread_run(monitor_connect_thread, NULL, "gadget");
	if (IS_ERR(gadget_task))                            
   ...
   gfp_t gfp_flags)
{
	...
	if(likely(_req))
	{
		...
		if(queue && &_req->queue != queue->next)
...
static void __ep_rx(int ep_num)
{
	...
	if((size != ep->maxpacket) || (req->req.actual == req->req.length))
	{
		...
		if(!g_rx_flag[ep_num])
 		...
 		__req_done(ep, req, 0);
 		dprintk(0, "rc %d %d",g_rx_flag[ep_num],size);
	}
