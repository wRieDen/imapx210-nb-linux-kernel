...
static void s3c24xx_spi_gpiocs(struct s3c2410_spi_info *spi, int cs, int pol)
{
	...
	if (pol)
	{
#ifdef CONFIG_TP_TI2046_CS
		imapx_gpio_setpin(tp_ti2046_cs, 0, IG_NORMAL);
#endif
	}
	else
	{
#ifdef CONFIG_TP_TI2046_CS
		imapx_gpio_setpin(tp_ti2046_cs, 1, IG_NORMAL);
#endif
	}
}
...
/*
	// enable ssi0, cs0 
	imapx_gpio_setcfg(IMAPX_GPE_RANGE(4, 6), IG_CTRL0, IG_NORMAL);
	imapx_gpio_setcfg(IMAPX_GPE7, IG_OUTPUT, IG_NORMAL);
*/
#ifdef CONFIG_TP_TI2046_SSI
	switch (CONFIG_TP_TI2046_SSI)
	{
		case 0:
			imapx_gpio_setcfg(IMAPX_GPE7, IG_OUTPUT, IG_NORMAL);
			imapx_gpio_setcfg(IMAPX_GPE6, IG_CTRL0, IG_NORMAL);
			imapx_gpio_setcfg(IMAPX_GPE5, IG_CTRL0, IG_NORMAL);
			imapx_gpio_setcfg(IMAPX_GPE4, IG_CTRL0, IG_NORMAL);
			break;
		case 1:
			break;
		case 2:
...
#endif
#ifdef CONFIG_TP_TI2046_CS
	tp_ti2046_cs = __imapx_name_to_gpio(CONFIG_TP_TI2046_CS);
	if(tp_ti2046_cs == IMAPX_GPIO_ERROR) {
		printk(KERN_ERR "failed to get tp_ti2046_cs pin.\n");
		return -1;
	}   
	imapx_gpio_setcfg(tp_ti2046_cs, IG_OUTPUT, IG_NORMAL);
#endif
...
static struct ads7846_platform_data ads_info = {                                                                        
	.model                  = 7846,                                             
	//.pressure_max           = 1400,//15000,1000(infoTM)                                                           
	.swap_xy                = 1,                                  
