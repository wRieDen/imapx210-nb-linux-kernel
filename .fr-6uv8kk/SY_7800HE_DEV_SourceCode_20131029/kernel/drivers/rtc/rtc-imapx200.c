...
static struct timespec time_suspend;
...
{
	if (set_jiffies && time_before(jiffies, set_jiffies+(2*HZ)))
	{
		...
		rtc_tm->tm_wday = rtc_buffer.tm_wday;
...
static int imap_rtc_settime(struct device *dev, struct rtc_time *tm)
{
	...
	set_jiffies = jiffies;
	rtc_buffer.tm_sec	= ttm.tm_sec;
	rtc_buffer.tm_min	= ttm.tm_min;
	rtc_buffer.tm_hour	= ttm.tm_hour;
	rtc_buffer.tm_mday	= ttm.tm_mday;
	rtc_buffer.tm_wday	= ttm.tm_wday;
	rtc_buffer.tm_mon	= ttm.tm_mon;
	rtc_buffer.tm_year	= ttm.tm_year;
...
static const struct rtc_class_ops imap_rtcops = {
	...
	.set_alarm			=	NULL,//imap_rtc_setalarm,
	.proc				=	imap_rtc_proc,
...
while(!tm.tm_sec)
	...
	/* Check if RTC wake up, if so close the Machine */
		printk("***************time.tv_sec = %d,CONFIG_PM_SHUT_DOWN_SEC+ time_suspend.tv_sec - 60 = %d \n",\
				time.tv_sec,CONFIG_PM_SHUT_DOWN_SEC+ time_suspend.tv_sec - 60);
		if(time.tv_sec >= CONFIG_PM_SHUT_DOWN_SEC+ time_suspend.tv_sec - 60)
		{
	if(readl(rWP_ST) & 0x2)
	{
		/* first clear this wp status */
		writel(readl(rWP_ST) | 0x2, rWP_ST);
		...
		else
		{
			...
			/* shut down */
			writel(0xff, rWP_MASK);
			writel(0x4, rGPOW_CFG);
		}
	}
}
#endif
...
{
	printk(KERN_INFO "iMAPx200 RTCv2, (c) 2009, 2014 InfoTM Microelctronics Co., Ltd\n");
