...
#define CONFIG_ODD_SCREEN_SHIFT 0

static unsigned int		*overlay_reg	= NULL;	/* overlay control register added by sololz */

#ifdef CONFIG_LCD_ENABLE_IRQ
#define VFP_DELAY       (imapfb_fimd.pixclock * \
		(imapfb_fimd.xres+imapfb_fimd.hsync_len+imapfb_fimd.left_margin+imapfb_fimd.right_margin) * \
		(imapfb_fimd.lower_margin) / \
		1000000)
...
void imapfb_set_gpio(int on)
{
	int i;

	imapx_gpio_setcfg(IMAPX_GPM_ALL, (on ?IG_CTRL0: IG_INPUT), IG_NORMAL);
	imapx_gpio_pull(IMAPX_GPM_ALL, (on ?0: 1), IG_NORMAL);

	imapx_gpio_setcfg(IMAPX_GPN_ALL, (on ?IG_CTRL0: IG_INPUT), IG_NORMAL);
	imapx_gpio_pull(IMAPX_GPN_ALL, (on ?0: 1), IG_NORMAL);
}

static void imapfb_set_lcd_power(UINT32 on)
{
	uint32_t v33, avdd, vgl, vgh, bl;

	v33 = __imapx_name_to_gpio(CONFIG_IG_LCD_33);
	avdd = __imapx_name_to_gpio(CONFIG_IG_LCD_AVDD);
	vgl = __imapx_name_to_gpio(CONFIG_IG_LCD_VGL);
	vgh = __imapx_name_to_gpio(CONFIG_IG_LCD_VGH);

	if(v33 != IMAPX_GPIO_ERROR) {
		imapx_gpio_setcfg(v33, IG_OUTPUT, IG_NMSL);
	}
	if(avdd != IMAPX_GPIO_ERROR) {
		imapx_gpio_setcfg(avdd, IG_OUTPUT, IG_NMSL);
	}
	if(vgl != IMAPX_GPIO_ERROR) {
		imapx_gpio_setcfg(vgl, IG_OUTPUT, IG_NMSL);
	}
	if(vgh != IMAPX_GPIO_ERROR) {
		imapx_gpio_setcfg(vgh, IG_OUTPUT, IG_NMSL);
	}

	if(on)
	{
		if(v33 != IMAPX_GPIO_ERROR) {
	//		imapx_gpio_chmod(v33, IG_NORMAL);
	//		imapx_gpio_setpin(v33, 1, IG_NORMAL);
	//		mdelay(25);
		}
		if(avdd != IMAPX_GPIO_ERROR) {
			imapx_gpio_chmod(avdd, IG_NORMAL);
			imapx_gpio_setpin(avdd, 1, IG_NORMAL);
		}
		if(vgl != IMAPX_GPIO_ERROR) {
			imapx_gpio_chmod(vgl, IG_NORMAL);
			imapx_gpio_setpin(vgl, 1, IG_NORMAL);
			msleep(15);
		}
		if(vgh != IMAPX_GPIO_ERROR) {
			imapx_gpio_chmod(vgh, IG_NORMAL);
			imapx_gpio_setpin(vgh, 1, IG_NORMAL);
		}
		...
		msleep(1);
		if(vgh != IMAPX_GPIO_ERROR) {
			imapx_gpio_setpin(vgh, 0, IG_NMSL);
			imapx_gpio_chmod(vgh, IG_SLEEP);
		}
		msleep(1);
		if(avdd != IMAPX_GPIO_ERROR) {
			imapx_gpio_setpin(avdd, 0, IG_NMSL);
			imapx_gpio_chmod(avdd, IG_SLEEP);
		}
		if(vgl != IMAPX_GPIO_ERROR) {
			imapx_gpio_setpin(vgl, 0, IG_NMSL);
			imapx_gpio_chmod(vgl, IG_SLEEP);
		}
		msleep(1);
		if(v33 != IMAPX_GPIO_ERROR) {
	//		imapx_gpio_setpin(v33, 0, IG_NMSL);
	//		imapx_gpio_chmod(v33, IG_SLEEP);
		}
	}
}
...
{
	/* this is not supported at this level */
...
/*****************************************************************************
**- Description
** 	The func is used to check is the kernel logo is replaced in the apk
**-Retren 
**	1 :yes 		0 :No
*****************************************************************************/

static int new_logo = 0, old_logo = 0;
...
{
	unsigned char *new_logo_data, *logo_data = NULL;
...
//			fbi->win_id, fbi->map_cpu_f1, fbi->map_size_f1);
		...
		//Show kernel logo
		if ( fbi->win_id == 1 ){
			new_logo_data = (unsigned char*)(ioremap(0x4a000000, 	fbi->fb.var.xres_virtual *
										fbi->fb.var.yres_virtual *
										imapfb_fimd.bytes_per_pixel));
			if(!new_logo_data){
				printk("Error: ioremap failed for new_logo_data\n");
			}
			new_logo = new_logo_exist( new_logo_data );
			printk(KERN_INFO "new_logo = %d\n",new_logo);
#if defined (CONFIG_IG_LOGO_ALT)
			old_logo = 1;
#endif
			if (new_logo) {
				logo_data = new_logo_data;		//logo new you add in the apk
			}else if (old_logo) { 
				logo_data = imapfb_oem_logo_data;	//logo old like bird
			}else{
				logo_data = NULL;			//logo such as penguin
			}

			if (logo_data){
				memcpy(fbi->map_cpu_f1, logo_data,
					  fbi->fb.var.xres_virtual *
					  fbi->fb.var.yres_virtual *
					  imapfb_fimd.bytes_per_pixel);
			}
			if(new_logo_data) 
				iounmap(new_logo_data);	
		}else{
...
static void imapfb_set_fb_addr(const imapfb_info_t *fbi)
{
	...
	switch (fbi->win_id)
	{
		case 0:
			...
			writel(imapfb_fimd.ovcw0b0sar + CONFIG_ODD_SCREEN_SHIFT, IMAP_OVCW0B0SAR);
	        	break;

		case 1:
			imapfb_fimd.ovcw1b0sar = start_address;
			writel(imapfb_fimd.ovcw1b0sar + CONFIG_ODD_SCREEN_SHIFT, IMAP_OVCW1B0SAR);
			break;

		case 2:
			imapfb_fimd.ovcw2b0sar = start_address;
			writel(imapfb_fimd.ovcw2b0sar + CONFIG_ODD_SCREEN_SHIFT, IMAP_OVCW2B0SAR);
		        break;

		case 3:
			imapfb_fimd.ovcw3bsar = start_address;
			writel(imapfb_fimd.ovcw3bsar + CONFIG_ODD_SCREEN_SHIFT, IMAP_OVCW3BSAR);
			break;
...
//#ifdef CONFIG_IG_LOGO_ALT
int imapfb_shutdown_fb(int win_num)
{
    printk("-----------imapfb_shutdown_fb");
		writel(readl(IMAP_OVCW0CR + (0x80 * win_num)) & ~IMAP_OVCWxCR_ENWIN_ENABLE, IMAP_OVCW0CR + (0x80 * win_num));
		return 0;
}
EXPORT_SYMBOL(imapfb_shutdown_fb);
//#endif
...
//	chan >>= 32 - bf.length;
	/*******************************************************************
	 * change by Besto: Fix the bug in text mode when bpp equal 32
	 ********************************************************************/
	chan >>= bf.length;
#endif
...
{
    printk("----------imapfb_set_win_coordinate");
...
#ifdef CONFIG_IG_LCD_EDID
	adapter = i2c_get_adapter(CONFIG_IG_LCD_EDID_I2C + 1);
#endif
	if (!adapter)
	{
...
//			writel(imapfb_fimd.lcdcon1, IMAP_LCDCON1);
		...
		case 1:
			...
			//if the logo is bird or new,enable win1,it will be disalbe in init/main.c
...
#endif
int early_suspend = 0;
EXPORT_SYMBOL(early_suspend);
void imapfb_early_suspend(struct early_suspend *h)
{
	imapfb_info_t *info = container_of(h, imapfb_info_t, early_suspend);
	unsigned int temp;
	printk(KERN_INFO"imapfb_early_suspend!\n");
#if defined(CONFIG_DOUBLE_DISPLAY_MODE) || defined(CONFIG_PANAUISION_MODE)
	/*add to avoid bug*/
	if(DOUBLE_DISPLAY_MODE == 1 || PANAUISION_MODE == 1 || VIDEO_MODE == 1)
		return;
#endif
...
void imapfb_early_resume(struct early_suspend *h)
{
	imapfb_info_t *info = container_of(h, imapfb_info_t, early_suspend);
	unsigned int temp, i;
	printk(KERN_INFO"imapfb_early_resume!\n");

    printk("---------------enter imapfb_early_resume \n ");

#if defined(CONFIG_DOUBLE_DISPLAY_MODE) || defined(CONFIG_PANAUISION_MODE)
	/*add to avoid bug*/
	if(DOUBLE_DISPLAY_MODE == 1 || PANAUISION_MODE == 1 || VIDEO_MODE == 1 || early_suspend == 0)
		return;
#endif
...
//	imapfb_set_backlight_level(IMAPFB_DEFAULT_BACKLIGHT_LEVEL);
	...
	info->early_suspend.suspend = imapfb_early_suspend;
	info->early_suspend.resume= imapfb_early_resume;
...
{
    printk("----------imapfb_suspend");
	struct fb_info *fbinfo = platform_get_drvdata(dev);
	imapfb_info_t *info = fbinfo->par;
	unsigned int temp;
	printk(KERN_INFO"imapfb_suspend!\n");
	imapbl_lowlevel_blctrl(0);
	msleep(100);
	imapfb_stop_lcd();
	imapfb_set_lcd_power(0);
...
int imapfb_resume(struct platform_device *dev)
{
	...
	imapfb_info_t *info = fbinfo->par;
	unsigned int temp, i;
	printk(KERN_INFO"imapfb_resume!\n");
    printk("---------------enter imapfb_resume \n ");
#if 0

	/*software reset ids subsystem*/
...
//	printk("%s line%d\n",__func__,__LINE__);
	...
	//msleep(100);
	msleep(500);
    //imapbl_lowlevel_blctrl(1); firstin
#endif
