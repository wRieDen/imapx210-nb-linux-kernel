...
static int imap_nand_resume(struct platform_device *pdev)
{
	...
	imap_nand.chip->cmdfunc(imap_nand.mtd, NAND_CMD_RESET, -1, -1);

	/* Pull nCE0 high to disable NAND chip */
	imap_nand.chip->select_chip(NULL, -1);
...
static int __init imap_nand_init(void)
{
	...
	printk(KERN_INFO "Method: IRQ(%d), DMA(%d), SECC(%d), DEC.RDY, CRCD++, Double\n",
