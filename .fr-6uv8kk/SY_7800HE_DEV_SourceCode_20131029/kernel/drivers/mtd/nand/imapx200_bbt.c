...
int imap_scan_bbt(struct mtd_info *mtd)
{
	...
	imap_blk_count = chip->chipsize >> chip->bbt_erase_shift;

	imap_bbt = kzalloc(imap_blk_count, GFP_KERNEL);
	if(imap_bbt == NULL){
		__ibbt_msg("Allocating %x for imap BBT failed!\n", imap_blk_count);
		goto __nor_bbt_failed__;
	}

	count = chip->chipsize >> chip->bbt_erase_shift;
