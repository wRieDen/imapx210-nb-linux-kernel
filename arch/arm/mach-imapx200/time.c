...
static void imapx200_timer_setup (void)
{
	...
	unsigned long pclk;
	unsigned long sclk_gate;
	int i = 0;
reload:
	tcnt = 0xffffffff;  /* default value for tcnt */
	sclk_gate = __raw_readl(rSCLK_MASK);
	sclk_gate |= IMAP_CLKCON_SCLK_TIM0;
	__raw_writel(sclk_gate, rSCLK_MASK);
...
//	printk("timer tcr0=%08lx, tcnt %08lx, usec %08lx\n", tcr0, tcnt, timer_usec_ticks);
	...
	if(__raw_readl(rTimer0CurrentValue) > __raw_readl(rTimer0LoadCount))
	{
		printk(KERN_ERR "Oops: Poweroverwhelming!\n");
		goto reload;
	}
}
