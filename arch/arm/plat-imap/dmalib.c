...
//		} 
		...
		/* 
		 * FIXME: By Sun
		 * Origin function call is dmac_clean_range(), modified to dmac_flush_range() while
		 * kernel transplanting. To be checked the validation and optimized.
		 */
		dmac_flush_range(chan->plli, chan->plli + pclient->block_num * sizeof(struct dma_lli)); 	
	return 1;
}
