	...
	unsigned int value)
{
	...
	{
		codec->hw_write(codec->control_data, data, 2);//add to try xfer again,it may lead more dbg msg
...
//	printk(KERN_ALERT "--------------------------suspend uses imap_iokey_spken(0)\n");
//	imap_iokey_spken(0);
...
//	printk("*********now, init every register!\n");
	...
	printk(KERN_ALERT "start writing ES8328_CHIPPOWER");	
	//es8328_write(codec, ES8328_CHIPPOWER, 0x00); //0x02
	
	if(es8328_write(codec, ES8328_CHIPPOWER, 0x00)){
		printk(KERN_INFO "ES8328_CHIPPOWER write fail twice,try once more.\
				if xfer err,it ain't have no voice");
		es8328_write(codec, ES8328_CHIPPOWER, 0x00);
	}

//	imap_iokey_spken(1);
	printk(KERN_INFO"YYYYYYYYYYYYYYY\n");
	return 0;
}
