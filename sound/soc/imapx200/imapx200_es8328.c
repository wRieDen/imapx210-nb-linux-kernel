...
//	iisclk_dev = (IMAPX200_IIS_CLK - );
	...
	ret = codec_dai->ops->set_sysclk(codec_dai, ES8328_MCLK, pll_out,
		SND_SOC_CLOCK_IN);
	if (ret < 0)
	...
	ret = codec_dai->ops->set_clkdiv(codec_dai, ES8328_BCLKDIV, bclk);
	if (ret < 0)
		{ printk("aaa4\n");
		return ret;
		}
...
static const struct snd_kcontrol_new es8328_imapx200_controls[] = {
#if 0
 ...
 */
static int imapx200_es8328_init(struct snd_soc_codec *codec)
{
	int i, err;
...
//	set_scenario_endpoints(codec, IMAPX200_AUDIO_OFF);
	...
	for (i = 0; i < ARRAY_SIZE(es8328_imapx200_controls); i++) {
		err = snd_ctl_add(codec->card,
				snd_soc_cnew(&es8328_imapx200_controls[i],
				codec, NULL));
		if (err < 0)
			return err;
	}
...
static struct es8328_setup_data imapx200_es8328_setup = {
	.i2c_address = 0x10 + CONFIG_CODEC_ES8328_CE,
	.i2c_bus = CONFIG_CODEC_ES8328_I2C + 1,
};
...
//	.platform = &s3c24xx_soc_platform,
	...
	.codec_data = &imapx200_es8328_setup,
};
...
static int __init imapx200_init(void)
{
	...
	tmp = __imapx_name_to_gpio(CONFIG_CODEC_ES8328_POWER);

	if(tmp == IMAPX_GPIO_ERROR)
	  printk(KERN_ERR "get es8328 power pin failed. may be not configurated.\n");
	else
	{
...
MODULE_DESCRIPTION("ALSA SoC ES8328 IMAPX200");
MODULE_LICENSE("GPL");
