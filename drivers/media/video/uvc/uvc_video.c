...
static void mMemcpy(void* dest, void* src, int size)
{
#if 0
...
#else
	uint64_t* d = (uint16_t*)dest;
	uint64_t* s = (uint16_t*)src;
	uint8_t* td = NULL;
	uint8_t* ts = NULL;
	while (size > 7) {
		*(d++) = *(s++);
	...
	struct uvc_buffer *buf)
{
	...
	if (stream->bulk.header_size == 0 && !stream->bulk.skip_payload) {
		...
		if (ret < 0 || buf == NULL) {
			...
			mMemcpy(stream->bulk.header, mem, ret);
