...
extern void ___dma_single_cpu_to_dev(const void *kaddr, size_t size,
...
void gmac_cre_def_tx_des(volatile unsigned int *tx_des_base_addr)
{
	...
	___dma_single_cpu_to_dev((const void *)tx_des_base_addr, SIZE_OF_TXDES, DMA_TO_DEVICE);
}
...
void gmac_cre_def_rx_des(volatile unsigned int *rx_des_base_addr)
{
	...
	___dma_single_cpu_to_dev((const void *)rx_des_base_addr, SIZE_OF_RXDES, DMA_TO_DEVICE);
}
...
void gmac_cre_cur_tx_des(struct board_info *db, uint32_t *pdata_phyaddr, uint32_t len, volatile unsigned int *tx_des_base_addr)
{
	...
	*(pointto_cur_des + 1) |= FS | TXLS | len;     //the first and last segment of a frame is in one descriptor
	___dma_single_cpu_to_dev((const void *)pointto_cur_des, SINGLE_TXDES_SIZE, DMA_TO_DEVICE);
#endif
}
...
static void gmac_rx(struct net_device *dev)
{
	...
	___dma_single_cpu_to_dev((const void *)pointto_cur_des, SINGLE_RXDES_SIZE, DMA_FROM_DEVICE) ;
	...
	while (readl(pointto_cur_des) & RXLS)			//the RXLS bit means the end of one received frame
	{
		...
		___dma_single_cpu_to_dev((const void *)pointto_cur_rx_pkt, pkt_len, DMA_FROM_DEVICE) ;
...
#if defined(CONFIG_IMAPX_USE_MMPOOL_TO_ACCELERATE_NET_SPEED)
		if ((skb = __dev_alloc_skb(pkt_len + 2, 0x2b2b2b2b)) != NULL)
#else
		...
		___dma_single_cpu_to_dev((const void *)pointto_cur_des, SINGLE_RXDES_SIZE, DMA_TO_DEVICE);
...
static void gmac_release_board(struct platform_device *pdev, struct board_info *db)
{
	...
	kfree((const void *)db->eth_base_txdes_vir);
...
static int gmac_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	___dma_single_cpu_to_dev((const void *)skb->data, skb->len, DMA_TO_DEVICE);
...
void gmac_cfg_gpio(void)
{
	...
	/*change for reset*/
	imapx_gpio_setcfg(IMAPX_GPP8, IG_OUTPUT, IG_NORMAL);
	/*write 1 to asigned gpio (gpio data only 0 and 1 is valid)*/
	imapx_gpio_setpin(IMAPX_GPP8, 1, IG_NORMAL);
}
...
static int __devinit gmac_probe(struct platform_device *pdev)
{
	...
	db->eth_base_txdes_vir = (volatile unsigned int *)kmalloc(SIZE_OF_DMA_SPA, GFP_KERNEL);
	ETH_BASE_TXDES_PHY =  virt_to_phys((void *)db->eth_base_txdes_vir);
