			    ...
			    __u8 *data, int interrupt)
{
	...
	for (n = 0; n < count; n++) {
	//	printk(KERN_ALERT "field->physical =%d,maxusage is %d,flags is %d,report_count = %d,report_offset=%d",field->physical,field->maxusage,field->flags,count,offset);
		if (HID_MAIN_ITEM_VARIABLE & field->flags) {
	//		printk(KERN_ALERT "hid_process_event");
...
//	printk(KERN_ALERT __LINE__,"report = report_enum->report_id_hash[n] =%d",report);
	if (!report)
		return;
