...
static void imapx_set_gpio(int channel,int width)
{
	switch(channel)
	{
		case 2:
			if(width == 4)
			{
				imapx_gpio_setcfg(IMAPX_GPO_RANGE(6, 11), IG_CTRL0, IG_NORMAL);
				imapx_gpio_pull(IMAPX_GPO_RANGE(6, 11), 0, IG_NORMAL);
			}
			else if (width == 0)
			{
...
/******************************************************************/
static struct mmc_host *sdhci_host2 = NULL;

void sdhci_mmc2_detect_change(void)
{
		if(sdhci_host2)
			mmc_detect_change(sdhci_host2,msecs_to_jiffies(100));
}
EXPORT_SYMBOL(sdhci_mmc2_detect_change);
/*****************************************************************/
