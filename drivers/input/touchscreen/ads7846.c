...
//	printk("~~~~~~~~~~~~~~~~~~ power1:%d, power2:%d ,power3:%d, power4:%d, power5:%d~~~~~~~~~~~~~~~~~~~~~~\n", z2, read_batt0, read_batt1, read_batt2, read_batt3);
	//printk(KERN_INFO "[battery_value]:%d\n",battery_val);
	if(batt_init_ok ==1)
        	supply_timer_func();
/*	for(i=0;i<50;i++)
		udelay(1000);
*/
	...
	hrtimer_start(&ts->timer, ktime_set(1, TS_POLL_PERIOD),\
			HRTIMER_MODE_REL);
}
