...
#ifdef CONFIG_BATT_EXTERNAL
#define	READ_BATT0(vref)   0xa7//a5	
#define	READ_BATT1(vref)   0xa7//5	
#define	READ_BATT2(vref)   0xa7//5	
#define	READ_BATT3(vref)   0xa7//5	
#define	READ_BATT4(vref)   0x90
#endif
...
unsigned int ti2046_int;

int battery_val;
EXPORT_SYMBOL(battery_val);
int ti_get_adc_val(void)
{
	return battery_val;
}
...
{
	return !(imapx_gpio_getpin(ti2046_int, IG_NORMAL));
}
...
static void imap_addc_rx(void *ads)
{
	...
	battery_val = (read_batt0 + read_batt1 + read_batt2 + read_batt3) / 4;
	//battery_val = (z2 + read_batt0 + read_batt1 + read_batt2 + read_batt3) / 5;
//	printk(KERN_EMERG"~~~~~~~~~~~~~z2:%d, power:%d ,power:%d, power:%d, power:%d, batt:%d~~~~~~~\n", z2, read_batt0, read_batt1, read_batt2, read_batt3, battery_val);

	//__raw_writel(tmp & ~(1<<5), rINTMSK);	
	hrtimer_start(&ts->timer, ktime_set(2, TS_POLL_PERIOD),\
			HRTIMER_MODE_REL);
}
...
static enum hrtimer_restart imap_addc_timer(struct hrtimer *handle)
{
	...
	int		status = 0;
	//printk(KERN_EMERG"*********imap_add_timer!\n");
	/* pen is still down, continue with the measurement */
	if (imapx_gpio_getpin(ti2046_int, IG_NORMAL)) {
		//printk(KERN_EMERG"********pen is up!\n");
		imapx_gpio_mask_irq(ti2046_int);	
		ts->msg_idx = 0;
		ts->wait_for_sync();
		status = spi_async(ts->spi, &ts->msg[0]);
		if (status)
			dev_err(&ts->spi->dev, "spi_async --> %d\n", status);
		imapx_gpio_unmask_irq(ti2046_int);
	} else {
		//printk(KERN_EMERG"********pen is down!\n");
...
#ifdef CONFIG_BATT_EXTERNAL
	ret = hrtimer_cancel(&ts_imap->timer);
	printk(KERN_EMERG"@@@@@%d\n",ret);
	while(!ret) {
		printk(KERN_EMERG"the timer was not in use...\n");
		msleep(1);
		ret = hrtimer_cancel(&ts_imap->timer);
	}
#endif
...
#ifdef CONFIG_BATT_EXTERNAL
	hrtimer_start(&ts_imap->timer, ktime_set(2, TS_POLL_PERIOD),\
			HRTIMER_MODE_REL);
#endif
	return 0;
}
...
static int __devinit ads7846_probe(struct spi_device *spi)
{
	...
	//config IRQ
	ti2046_int  = __imapx_name_to_gpio(CONFIG_TP_TI2046_INT);
	if(ti2046_int == IMAPX_GPIO_ERROR) {
		printk(KERN_ERR "failed to get ti2046_int pin.\n");
		return -1;
	}   
	spi->irq = imapx_gpio_to_irq(ti2046_int);
	if (!spi->irq) {
		dev_dbg(&spi->dev, "no IRQ?\n");
		return -ENODEV;
	}
	imapx_gpio_setcfg(ti2046_int, IG_INPUT, IG_NORMAL);
	imapx_gpio_setirq(ti2046_int, FILTER_MAX, IG_BOTH, 1);
...
{
#ifdef CONFIG_BATT_EXTERNAL
	__imapx_register_batt(ti_get_adc_val);
#endif
