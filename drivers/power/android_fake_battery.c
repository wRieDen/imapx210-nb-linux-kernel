...
static int batt2capacity(int batt_value)
{
	...
	else if (isacon())//charging: the adapter is on and batt is not full
	{
	...
	else//discharging: adapter is off and batt is not full
	{
...
#endif
	if(!batt){
	//	printk(KERN_INFO "[BATTERY VAL IS 0!!!!!!WRONG!!!!!]:Please check your battery connection");
		capacity_show = 120;
	}
	if (batt != 0)
	{
		if (count == 0)
		{
			...
			//***********************************************************************************		
			// Modified  to make capacity_show will never be 100 until ischargingfull
			// It is good when  battery region is badly set
				if(unlikely(ischargingfull() && (capacity_show >= 99))){
					capacity_show = 100;
				} else {
					capacity_show = (capacity_show+1 > 99) ? 99 : capacity_show+1;
				}
			//***********************************************************************************
				...
				//capacity_show = (capacity_show-1 < 1 ) ? 1 : capacity_show-1;
		...
		{
			count = count - 1;
			if(count && ischargingfull() && isacon()) {//y :when battery is full,make capacity_show quicker  get to 100%
				count = count - 1;
			}
		}
	}

	battery_state = (isacon()?((ischargingfull()&&(capacity_show==100))?
...
/*******************************************************/
	...
	imapx_gpio_pull(adp_in,0,IG_NORMAL);
...
android_bat_resume(struct platform_device *pdev) 
{
	printk(KERN_INFO"Enter %s\n",__func__);
#if 1
	if((battery_state != POWER_SUPPLY_STATUS_UNKNOWN) && (!isacon())){	//to avoid full->suspend->pull out->longtime->resume, the battery should not be 100%
		battery_state = POWER_SUPPLY_STATUS_UNKNOWN;
		battery_old_state = POWER_SUPPLY_STATUS_UNKNOWN;
		capacity_show = 120;
		capacity_old_show = 0;
